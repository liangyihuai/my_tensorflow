...
def deepnn(features, labels, model):
  """deepnn builds the graph for a deep net for classifying digits.
  Args:
    features: an input tensor with the dimensions (N_examples, 784), where 784 is the
  ...
  with tf.name_scope('reshape'):
      x_image = tf.reshape(features, [-1, 64, 64, 3])
...
def main(_):
  ...
  with tf.Session() as sess:
    ...
    for i in range(800):
      ...
      if i % 20 == 0:
        train_accuracy = accuracy.eval(feed_dict={x: X_mini_batch, y_: Y_mini_batch, keep_prob: 1.0})
        print('step %d, training accuracy %g' % (i, train_accuracy))
      train_step.run(feed_dict={x: X_mini_batch, y_: Y_mini_batch, keep_prob: 0.5})
