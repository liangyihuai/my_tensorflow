...
def forward_propagate(X, parameters, training=True, t=2):
    W1 = parameters['W1']
    b1 = parameters['b1']
    W2 = parameters['W2']
    b2 = parameters['b2']
    ...
    if not training:
        moving_average_mean1 = MOVING_AVERAGE['mean1']
        moving_average_std1 = MOVING_AVERAGE['std1']
        mean1 = MOVING_AVERAGE_BATA*moving_average_mean1 + (1-MOVING_AVERAGE_BATA)*mean1
        std1 = MOVING_AVERAGE_BATA*moving_average_std1 + (1-MOVING_AVERAGE_BATA)*std1
        mean1_corrected = mean1/(1-np.power(MOVING_AVERAGE_BATA, t))
        std1_corrected = std1/(1-np.power(MOVING_AVERAGE_BATA, t))
        mean1 = mean1_corrected
        std1 = std1_corrected
    ...
    if not training:
        moving_average_mean2 = MOVING_AVERAGE['mean2']
        moving_average_std2 = MOVING_AVERAGE['std2']
        mean2 = MOVING_AVERAGE_BATA*moving_average_mean2 + (1-MOVING_AVERAGE_BATA)*mean2
        std2 = MOVING_AVERAGE_BATA*moving_average_std2 + (1-MOVING_AVERAGE_BATA)*std2
        mean2_corrected = mean2/(1-np.power(MOVING_AVERAGE_BATA, t))
        std2_corrected = std2/(1-np.power(MOVING_AVERAGE_BATA, t))
        mean2 = mean2_corrected
        std2 = std2_corrected

        update_moving_average(std1, mean1, std2, mean2)
...
def do_predict(X, parameters):
    cache = forward_propagate(X, parameters, training=False)
